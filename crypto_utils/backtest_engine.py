...
class BacktestEngine:
    ...
    def __init__(self, 
        ...
        logger.info(f"回测引擎初始化完成 - 初始资金: {initial_capital}, 最大持仓: {max_position_size}")
    ...
    def run_backtest(self,
        ...
        -----------
        pairs_config : Dict
            交易对配置，格式：{(symbol1, symbol2): {'gamma': x, 'threshold': y}}
        ...
        Dict
            回测结果
        """
        logger.info(f"开始回测，共 {len(pairs_config)} 个交易对")
        ...
        for pair, config in pairs_config.items():
            ...
            positions1, positions2 = strategy._calculate_positions(
                symbol1, symbol2, gamma, threshold, self.max_position_size
            )
        ...
        # 计算组合指标
        portfolio_metrics = self._calculate_portfolio_metrics(all_pnl['total'])
    ...
    def _calculate_portfolio_metrics(self, pnl_series: pd.Series) -> Dict:
        """
        计算组合性能指标
    ...
    def _print_summary(self):
        ...
        print(f"最终资金:        ${self.initial_capital + metrics['total_return']:,.2f}")
    ...
    def plot_results(self, save_dir: Optional[str] = None):
        ...
        # 总体累计PnL
        ax1 = axes[0]
        self.results['pnl']['cumulative'].plot(ax=ax1, linewidth=2, color='blue')
        ...
        for pair in self.results['pair_results'].keys():
            ...
            if pnl_col in self.results['pnl'].columns:
                cumulative = self.results['pnl'][pnl_col].cumsum()
                cumulative.plot(ax=ax2, label=f'{symbol1}-{symbol2}', linewidth=1.5)
        ...
        cumulative_pnl = self.results['pnl']['cumulative']
        running_max = cumulative_pnl.expanding().max()
        drawdown = cumulative_pnl - running_max
